...
In the previous case, the `reverse_sign()` function has a single argument **number**, while its content is **number* -1**. However, the functions have a third argument: *Environment*. When the `reverse_sign()` function created, it was in the **global** *Environment*.
